...
- [ ] Initialize Turborepo monorepo: `apps/web`, `apps/worker`, `packages/db`, `packages/shared`, `packages/ui`
- [ ] Supabase project: Postgres + Auth + a private Storage bucket for resumes
- [ ] Prisma schema, first migration: `profiles`, `job_sources`, `jobs`, `matches`, `applications`, `cover_letters`, `status_events`
- [ ] Tailwind theme configured from the tokens in `CLAUDE.md` §8 (colors, spacing, radius, fonts)
- [ ] Redis (Upstash) + BullMQ skeleton in `apps/worker`
- [ ] Shared Anthropic API client wrapper in `packages/shared` (not duplicated per call site)
- [ ] `.env.example` scaffolded — no real secrets committed
...
- [ ] Resume upload (PDF) → Supabase Storage, signed URLs only, never public
- [ ] CV parsing → structured skills/experience JSON via Claude API
- [ ] Profile editor screen — review/correct parsed skills before matching runs against them
- [ ] Loading / Empty / Error / Populated states on the profile screen

### 1.2 Job Ingestion
- [ ] Greenhouse adapter (public Job Board API, GET only)
- [ ] Lever adapter
- [ ] Adzuna adapter
- [ ] Arbeitnow or RemoteOK adapter (remote-specific coverage)
- [ ] `job_sources` config table + a settings screen to add/disable a source
- [ ] Scheduled polling job per source (BullMQ), respecting each source's documented rate limits
- [ ] Dedup enforced via `content_hash` + the unique constraints from `CLAUDE.md` §7
- [ ] Stale-job check: confirm a job still exists/is open before generating a cover letter for it

### 1.3 Matching Engine
- [ ] Skill-overlap scoring against the parsed profile
- [ ] Embedding similarity scoring, combined with skill overlap into one 0–100 score
- [ ] `matched_skills` array populated per match — this powers "why this matched" in the UI
- [ ] Score badge UI, banded (e.g. 80+ strong / 50–79 partial / <50 weak), using the fixed status colors from `CLAUDE.md` §8

### 1.4 Jobs List & Detail
- [ ] `/jobs` — paginated, filterable by score / source / remote-type
- [ ] `/jobs/[id]` — full description, match breakdown, "Generate cover letter" CTA
- [ ] Loading / Empty / Error / Populated states on both screens

### 1.5 Cover Letter Generation
- [ ] Single prompt template (`modules/coverletters/prompts.ts`) combining CV + job description + `matched_skills`
- [ ] Editable draft UI
- [ ] "Regenerate" creates a new version rather than overwriting the current one

### 1.6 Application Tracking (Kanban)
- [ ] Status state machine: `Matched → Drafted → Applied → Interviewing → Rejected / Ghosted`, illegal transitions rejected at the function level
- [ ] Kanban board UI with the fixed status colors from `CLAUDE.md` §8
- [ ] `status_events` row inserted on every transition (one row per change — `from_status`, `to_status`, `changed_at`)
- [ ] Duplicate prevention surfaced in the UI itself (e.g. an "Already tracked" badge when a job already has an application row for this user) — not just enforced silently at the DB layer
